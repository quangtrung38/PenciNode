...
import {
  ...
  identityMatrix,
  matrixToCSS,
  calculateControlPointsFromMatrix,
  type Point,
...
export default function EditMockupPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Load existing mockup data
  useEffect(() => {
    const loadMockupData = async () => {
      try {
        setIsLoadingData(true);
        const response = await fetch(`/api/admin/mockups/${resolvedParams.id}`);
        const result = await response.json();

        if (!response.ok) {
          throw new Error(result.error || 'Failed to load mockup');
        }
        ...
        setFormData({
          ...
          background_color: mockup.background_color || '#ffffff',
          display: mockup.display ?? 1,
          isViewMain: mockup.isViewMain ?? 0,
        });

        // Set image and parse boxes
        if (mockup.image) {
          ...
          img.onload = () => {
            ...
            // Parse and set boxes from jsoncol after image dimensions are set
            if (mockup.jsoncol) {
              try {
                const jsonData = JSON.parse(mockup.jsoncol);
                if (jsonData.boxes && Array.isArray(jsonData.boxes)) {
                  setBoxes(jsonData.boxes);
                  // Set first box as active if boxes exist
                  if (jsonData.boxes.length > 0) {
                    setActiveBoxIndex(0);
                    // Calculate control points for first box based on saved matrix3d
                    const firstBox = jsonData.boxes[0];
                    if (firstBox && firstBox.imageShow.matrix3d) {
                      ...
                      const calculatedPoints = calculateControlPointsFromMatrix(
                        originalCorners,
                        firstBox.imageShow.matrix3d
                      );
                      // Keep original corners as baseline, show transformed positions as control points
                      setControlPoints(calculatedPoints);
                      setOriginalPositions(originalCorners);
                    } else {
                      ...
                      setControlPoints(defaultPoints);
                      setOriginalPositions(defaultPoints);
                    }
                  }
                }
              } catch (jsonError) {
                console.error('Error parsing jsoncol:', jsonError);
              }
            }
          };
          img.src = mockup.image;
        } else {
          // Parse boxes even if no image (fallback)
          if (mockup.jsoncol) {
            try {
              const jsonData = JSON.parse(mockup.jsoncol);
              if (jsonData.boxes && Array.isArray(jsonData.boxes)) {
      ...
      setBoxes((prevBoxes) => {
        ...
        if (activeBoxIndex < newBoxes.length) {
          ...
          if (activeBox) {
            ...
            console.log('Updated box:', newBoxes[activeBoxIndex]);
          }
        }
        return newBoxes;
      });
  ...
  // Handle box click to select active box
  const handleBoxClick = (boxIndex: number) => {
    setActiveBoxIndex(boxIndex);
    
    // Calculate control points based on saved matrix3d transform
    const selectedBox = boxes[boxIndex];
    if (selectedBox && selectedBox.imageShow.matrix3d) {
      // Original corner positions (default rectangle - always keep these as reference)
      const originalCorners = [
        { x: 0, y: 0 },
        { x: 0, y: imageDimensions.height },
        { x: imageDimensions.width, y: 0 },
      ...
      // Calculate where control points should be based on the saved transform
      const calculatedPoints = calculateControlPointsFromMatrix(
        originalCorners,
        selectedBox.imageShow.matrix3d
      );
      
      // Keep original corners as baseline, show current transformed positions as control points
      setControlPoints(calculatedPoints);
      setOriginalPositions(originalCorners); // Keep original rectangle as baseline for transform calculations
    } else {
      // Fallback to default positions if no matrix3d
      const defaultPoints = [
        { x: 0, y: 0 },
        { x: 0, y: imageDimensions.height },
        { x: imageDimensions.width, y: 0 },
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (isDragging !== null && canvasRef.current) {
      ...
      setControlPoints((prevPoints) => {
        ...
        console.log('Updating control points:', newPoints);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mockupData = {
        ...
        isViewMain: formData.isViewMain,
      };
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to update mockup');
      }
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6 dark:border-white/[0.05] dark:bg-white/[0.03]">
            ...
            <div className="flex gap-3 flex-wrap mb-4">
              ...
              <button
                ...
                className="inline-flex items-center px-4 py-2.5 bg-gradient-to-r from-red-500 to-red-600 text-white text-sm font-medium rounded-lg hover:from-red-600 hover:to-red-700 focus:ring-4 focus:ring-red-200 transition-all duration-200 shadow-sm border border-red-500"
              >
                <svg 
                  xmlns="http://www.w3.org/2000/svg" 
                  className="h-4 w-4 mr-2" 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2">
                ...
                {boxes.map((box, boxIndex) => {
                  ...
                  const transformCSS = matrixToCSS(box.imageShow.matrix3d);
                  
                  return (
                    <div key={box.id}>
                      {/* Simple overlay mockup box (giống mẫu cũ Laravel) */}
                      <div
                        className="absolute cursor-pointer flex items-center justify-center"
                        style={{
                          position: 'absolute' as const,
                          left: box.imageShow.styles.left,
                          top: box.imageShow.styles.top,
                          width: box.imageShow.styles.width,
                          height: box.imageShow.styles.height,
                      ...
                      >
                        <span className="text-white font-semibold text-lg pointer-events-none">
                          {boxIndex + 1}
                        </span>
                      </div>
                    </div>
