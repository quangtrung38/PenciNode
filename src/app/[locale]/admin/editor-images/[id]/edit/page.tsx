...
export default function EditEditorImagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Load existing editor image data
  useEffect(() => {
    const loadEditorImageData = async () => {
      try {
        setIsLoadingData(true);
        const response = await fetch(`/api/admin/editor-images/${resolvedParams.id}`);
        const result = await response.json();

        if (!response.ok) {
          throw new Error(result.error || 'Failed to load editor image');
        }
        ...
        setThumbnailPreview(image.img_thumb);

      } catch (err) {
        setError(err instanceof Error ? err.message : 'Failed to load editor image');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validation
      if (!imagePreview) {
        throw new Error('Vui lòng có hình ảnh chính');
      }

      if (!thumbnailPreview) {
        throw new Error('Vui lòng có thumbnail');
      }
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to update editor image');
      }
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
          Chỉnh sửa Hình ảnh Editor
        </h1>
        <p className="mt-2 text-gray-600 dark:text-gray-400">
          Cập nhật thông tin hình ảnh editor
        </p>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Hỗ trợ các định dạng: JPG, PNG, GIF, WebP (tối đa 10MB). Để trống nếu không muốn thay đổi.
              </p>
            </div>
