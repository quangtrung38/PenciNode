...
// PATCH - Toggle mockup display status
async function handleToggleMockupDisplay(
  ...
  try {
    ...
    const currentMockup = await (Mockup as any).findById(mockupId).select('display').lean();
    ...
    const newStatus = currentMockup.display === 1 ? 0 : 1;
    ...
    return NextResponse.json({
      success: true,
      message: `Mockup ${newStatus === 1 ? 'displayed' : 'hidden'} successfully`,
      mockup: updatedMockup,
    });
  } catch (error) {
    console.error('Error toggling mockup display:', error);
    return NextResponse.json(
...
export const PATCH = withAdminAuth(handleToggleMockupDisplay);
