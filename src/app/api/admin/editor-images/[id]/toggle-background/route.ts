...
// PATCH - Toggle editor image background status
async function handleToggleEditorImageBackground(
  ...
  try {
    ...
    const currentImage = await (EditorImage as any).findById(imageId).select('is_background').lean();
    ...
    // Toggle the background status (0 -> 1, 1 -> 0)
    const newStatus = currentImage.is_background === 1 ? 0 : 1;

    // Update editor image background status
    const rawUpdatedImage = await (EditorImage as any).findByIdAndUpdate(
      imageId,
      { is_background: newStatus },
      {
        new: true,
    ...
    return NextResponse.json({
      success: true,
      message: `Editor image ${newStatus === 1 ? 'set as background' : 'removed from background'} successfully`,
      image: updatedImage,
    });
  } catch (error) {
    console.error('Error toggling editor image background:', error);
    return NextResponse.json(
...
export const PATCH = withAdminAuth(handleToggleEditorImageBackground);