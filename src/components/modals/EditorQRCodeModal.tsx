...
interface EditorQRCodeModalProps {
  ...
  editingQR: EditorQRCode | null;
}

export default function EditorQRCodeModal({
  isOpen,
  onClose,
  onSuccess,
  editingQR,
  ...
  const uploadImageToCloudinary = async (file: File) => {
    ...
    formDataUpload.append('folder', 'editor-qrcodes');

    const uploadResponse = await fetch('/api/admin/editor-qrcodes/upload', {
      method: 'POST',
      body: formDataUpload,
    });
  ...
  const deleteOldImage = async (imageUrl: string) => {
    try {
      ...
      await fetch('/api/admin/editor-qrcodes/delete-image', {
        method: 'POST',
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        if (editingQR && editingQR.img) {
          await deleteOldImage(editingQR.img);
        }
      ...
      const url = editingQR
        ? `/api/admin/editor-qrcodes/${editingQR.id}`
        : '/api/admin/editor-qrcodes';
      ...
      if (!response.ok) {
        throw new Error(result.error || `Failed to ${editingQR ? 'update' : 'create'} QR code`);
      }

      // Call success callback with action and QR code data
      const action = editingQR ? 'update' : 'create';
      const qrData = result.editorQRCode || result.data;
      onSuccess(action, qrData);
        ...
        <Dialog.Panel className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Tên QR Code <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  name="name"
            ...
            >
              {isLoading ? (
                <div className="flex items-center">
                  ...
                  {editingQR ? 'Đang cập nhật...' : 'Đang tạo...'}
                </div>
              ) : (
                editingQR ? 'Cập nhật' : 'Tạo QR Code'
              )}
            </button>
          </div>
        </Dialog.Panel>
